...
use crate::{
    ...
    utils::{
        adjust, decay_debt, decimal_multiplication_in_256, get_bond_price, get_current_debt,
        get_debt_ratio, get_max_payout, get_payout_for, get_pending_payout, get_true_bond_price,
    },
};
...
pub fn pay_subsidy(deps: DepsMut, info: MessageInfo) -> StdResult<Response> {
    if CONFIGURATION.load(deps.storage)?.subsidy_router
...
pub fn deposit(
    ...
    let native_price = get_true_bond_price(config.clone(), state.clone(), bond_price);
    ...
    let mut bond_info = BOND_INFOS
        .load(
            deps.storage,
            deps.api.addr_canonicalize(&depositor)?.as_slice(),
        )
        .unwrap_or_default();
    ...
    let principal_asset_info = config.principal_token.to_normal(deps.api)?;
    let principal_asset = Asset {
        info: principal_asset_info.clone(),
        amount: amount_without_fee,
    };

    let custom_treasury = deps.api.addr_humanize(&config.custom_treasury)?;
    messages.push(principal_asset.into_msg(&deps.querier, custom_treasury.clone())?);

    messages.push(CosmosMsg::Wasm(WasmMsg::Execute {
        contract_addr: custom_treasury.to_string(),
        funds: vec![],
...
pub fn redeem(deps: DepsMut, env: Env, user: String) -> StdResult<Response> {
    let mut bond_info =
        BOND_INFOS.load(deps.storage, deps.api.addr_canonicalize(&user)?.as_slice())?;
    ...
    if bond_info.payout.is_zero() {
        BOND_INFOS.remove(deps.storage, deps.api.addr_canonicalize(&user)?.as_slice());
    } else {
